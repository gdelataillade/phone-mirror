...
text paste, Home/App Switcher controls, remote rotation, and automatic reconnect.
Intermittent video stalls remain under investigation. Audio, Wi-Fi and controlling
...
Losing app or window focus, resizing, rotation and explicit release clear held input. Stale or missing video disables control. The
app restarts a stalled session after one second without decoded output, with
...
configuration. It queries device orientation every 500 ms and when the codec
configuration changes; orientation travels with each frame across the native boundary.
...
The last checks a manual restart while live. These simulate failures; they do not replace physical cable and sleep/wake tests.
